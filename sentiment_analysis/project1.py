...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    # Your code here
    avg_loss = 0.0
    sample = len(feature_matrix)
    for i in range(sample):
        target = feature_matrix[i]
        label = labels[i]

        hinge_fn = float((np.dot(target, theta) + theta_0) * label)
        if hinge_fn <= 1:
            avg_loss += 1-hinge_fn

    avg_loss *= float(1.0/sample)
    return avg_loss
    raise NotImplementedError
#pragma: coderesponse end
...
def perceptron(feature_matrix, labels, T):
    ...
    """
    # Your code here
    current_theta = np.zeros(int(feature_matrix.size/len(labels)))
    current_theta_0 = 0
    for t in range(T):
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    # Your code here
    x= np.dot(feature_matrix, theta)
    y= x + theta_0
    sign_y = np.sign(y)
    return np.sign(sign_y - 0.5)
    raise NotImplementedError
#pragma: coderesponse end
